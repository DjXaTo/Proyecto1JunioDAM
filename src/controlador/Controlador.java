...
import vista.JDInsertProy;
import vista.JDNewEmpleado;
import vista.JDNewProyect;
...
public class Controlador {
    ...
    //BOTONES DE LOS EMPLEADOS
        ...
        p.btnNewEmple.addActionListener((ActionEvent) -> {
            ...
            jdne.btnAceptar.addActionListener((ActionEvent e) -> {
                ...
                consultas.agregarEmpleado(nif, nombre, apell, fch);
        ...
        p.btnDespedir.addActionListener((ActionEvent e) -> {
            ...
            if (p.TablaEmpleados.getSelectedRow() == -1) {
                ...
                consultas.borrarEmpleado(HELP.getId());
                p.TablaEmpleados.setModel(this.getTablaEmple());
            }
        });
    ...
    //BOTONES DE LOS PROYECTOS
        ...
        p.btnModProyecto.addActionListener((ActionEvent e) -> {
            jdnp = new JDNewProyect(p, true);
            proyectos = consultas.leerProyectos();
            Proyecto auxi = proyectos.get(p.jComboBox1.getSelectedIndex());
        ...
        p.btnDeleteProyecto.addActionListener((ActionEvent e) -> {
            proyectos = consultas.leerProyectos();
            Proyecto HALP = proyectos.get(p.jComboBox1.getSelectedIndex());
            consultas.borrarProyecto(HALP.getId());
            p.jComboBox1.setModel(this.getComboBox());
            this.setTxtsProyecto(proyectos.get(p.jComboBox1.getSelectedIndex()));
        });
        ...
        p.btnEliminaRelacion.addActionListener((ActionEvent e) -> {
            proyectos = consultas.leerProyectos();
            empleados = consultas.recogerEmpleProy(proyectos.get(p.jComboBox1.getSelectedIndex()).getId());
    ...
    public DefaultTableModel getTablaEmpleProy() {
        ...
        ArrayList<Empleado> recogerEmpleados = consultas.recogerEmpleProy(proyectos.get(p.jComboBox1.getSelectedIndex()).getId());
        for (Empleado e : recogerEmpleados) {
            fila[0] = e.getNif();
            fila[1] = e.getNombre();
            fila[2] = e.getApellido();
